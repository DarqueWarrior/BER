...
if ($installDep.IsPresent -or $analyzeScript.IsPresent) {
   ...
   $manifest = Import-PowerShellDataFile .\Source\Trackyon.Ber.psd1
...
Copy-Item -Path ./Source/Trackyon.Ber.psm1 -Destination "$output/Trackyon.Ber.psm1" -Force
...
(Get-Content "./Source/Trackyon.Ber.psd1") -Replace ("FunctionsToExport.+", "FunctionsToExport = ($newValue)") |
   Set-Content "$output/Trackyon.Ber.psd1"

Write-Output "Publish complete to $output"

if ($ipmo.IsPresent -or $runTests.IsPresent) {
   Import-Module "$output/Trackyon.Ber.psd1" -Force
}
...
if ($analyzeScript.IsPresent) {
   ...
   $r = Invoke-ScriptAnalyzer -Path $output -Recurse | Where-Object severity -ne "Information"
...
if ($runTests.IsPresent) {
   ...
   if ($codeCoverage.IsPresent) {
      Invoke-Pester -Script .\unit -CodeCoverage .\dist\*.ps1 -CodeCoverageOutputFile coverage.xml -CodeCoverageOutputFileFormat JaCoCo -Strict -OutputFile test-results.xml -OutputFormat NUnitXml
   }
   else {
      Invoke-Pester -Script .\unit -Strict -OutputFile test-results.xml -OutputFormat NUnitXml -passThru
   }
}